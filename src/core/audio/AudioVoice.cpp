...
void AudioVoice::Start(const SampleZone *zone, uint8_t pitch,
                       uint8_t velocity) {
  if (!zone || !zone->isLoaded || zone->audioData.empty())
    return;
  ...
  // If the velocity is extremely high, we boost the attack transient
  // mathematically
  if (velocity > 110) {
...
float AudioVoice::GetStealingPriority() const {
  if (state_.load(std::memory_order_acquire) == State::Free)
    return 0.0f;
  if (state_.load(std::memory_order_acquire) == State::Release)
    return 0.1f;
...
void AudioVoice::RenderNextBlock(float **outputChannels, uint32_t numFrames,
                                 uint32_t numOutputChannels) {
  auto currentState = state_.load(std::memory_order_acquire);
  if (currentState == State::Free || !activeZone_ ||
      activeZone_->audioData.empty()) {
    return;
  }
  ...
  for (uint32_t i = 0; i < numFrames; ++i) {
    ...
    for (uint32_t chan = 0; chan < numOutputChannels; ++chan) {
      // If sample is mono, read from channel 0. If stereo, read matching
