...
private:
    // Maps a dense block chord into sectionally-appropriate divisi (Vln1, Vln2, Vla, Vc, Cb)
    std::vector<std::pair<uint8_t, uint8_t>> AllocateDivisi(const ActiveChordContext& targetChord) const;
    
    // Pre-calculates CC11 (Expression) swells reading ahead to the next chord transition
    void CalculatePredictiveExpressionCurve(
