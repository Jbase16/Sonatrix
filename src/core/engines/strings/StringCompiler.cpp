...
MIDI::MIDIStream StringCompiler::CompileClip(
  ...
  for (const auto &mir : clip.basePattern->events) {
    if (mir.type != ArticulationType::GenericNote &&
        mir.type != ArticulationType::StringSwell) {
      continue;
    }
    ...
    for (const auto &[channel, pitch] : divisiAllocation) {
      stream.events.push_back({triggerTime, MIDI::MIDIEventType::NoteOn,
                               channel, pitch, mir.velocityBase});
      stream.events.push_back(
          {offTime, MIDI::MIDIEventType::NoteOff, channel, pitch, 0});
    }
...
std::vector<std::pair<uint8_t, uint8_t>> StringCompiler::AllocateDivisi(
    const ActiveChordContext & /*targetChord*/) const {
...
void StringCompiler::CalculatePredictiveExpressionCurve(
    MIDI::MIDIStream &outStream, MusicalTime chordStartTime,
    MusicalTime chordEndTime) const {
  // Produces a smooth curve of CC11 (Expression) events spanning the time
  // window. The curve scales exponentially the closer we get to chordEndTime.
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    uint8_t ccVal = static_cast<uint8_t>(
        (static_cast<float>(i) / static_cast<float>(steps)) * 127.0f);

    outStream.events.push_back({ccTime, MIDI::MIDIEventType::ControlChange, 1,
                                11, ccVal}); // Ch 1 Vlns
    outStream.events.push_back(
        {ccTime, MIDI::MIDIEventType::ControlChange, 2, 11, ccVal});
    outStream.events.push_back(
        {ccTime, MIDI::MIDIEventType::ControlChange, 3, 11, ccVal});
    outStream.events.push_back(
        {ccTime, MIDI::MIDIEventType::ControlChange, 4, 11, ccVal});
  }
}
