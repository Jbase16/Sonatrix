...
MIDI::MIDIStream PianoCompiler::CompileClip(
    ...
    for (const auto& mir : clip.basePattern->events) {
        if (mir.type != ArticulationType::GenericNote && mir.type != ArticulationType::PianoChord) {
            continue;
        }
        ...
        for (uint8_t currentNote : currentVoicing) {
            stream.events.push_back({triggerTime, MIDI::MIDIEventType::NoteOn, 0, currentNote, mir.velocityBase});
            
            MusicalTime offTime = triggerTime + BeatsToTime(mir.lengthBeats);
            stream.events.push_back({offTime, MIDI::MIDIEventType::NoteOff, 0, currentNote, 0});
        }
    }
    
    // 2. Automated Pedal Pass (CC64)
    SynthesizePedal(stream, clip.timelineStart, clip.timelineStart + BeatsToTime(4.0), chordTimeline);
...
void PianoCompiler::SynthesizePedal(
    ...
    outStream.events.push_back({startTime, MIDI::MIDIEventType::ControlChange, 0, 64, 127});
}
